...
double EdgeWeightCalculator<T>::compute_feature_similarity(int u, int v) {
    const int num_features = graph.get_num_features();
    int num_similar_features = 0;
    for (int feature_idx = 0; feature_idx < num_features; feature_idx++) {
        if (have_similar_feature(u, v, feature_idx)) num_similar_features++;
    }
    return static_cast<double>(num_similar_features) / static_cast<double>(num_features);
}
...
bool EdgeWeightCalculator<T>::have_similar_feature(int node1, int node2, int feature_idx) {
    ...
    constexpr double tolerance = std::is_floating_point_v<T> ? 0.0001 : 0;
...
double EdgeWeightCalculator<T>::compute_structural_similarity(
    int u, int v, const std::vector<std::unordered_set<int>>& covers) {
    const auto& cover_u = covers[u];
    const auto& cover_v = covers[v];

    const auto& smaller = (cover_u.size() < cover_v.size()) ? cover_u : cover_v;
    const auto& larger = (cover_u.size() < cover_v.size()) ? cover_v : cover_u;

    int intersection_size = 0;
    for (int elem : smaller) {
        if (larger.count(elem)) ++intersection_size;
    }
